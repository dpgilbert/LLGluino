...
void MT2Looper::loop(TChain* chain, std::string sample, std::string output_dir){
  ...
  if ((doScanWeights || applyBtagSF) && ((sample.find("T1") != std::string::npos) || (sample.find("T2") != std::string::npos) || (sample.find("T6") != std::string::npos) || (sample.find("ctau") != std::string::npos) )) {
    std::string scan_name = sample;
    if (sample.find("T1") != std::string::npos && sample.find("ctau") == std::string::npos) scan_name = sample.substr(0,6);
    ...
    else if (sample.find("ctau") != std::string::npos) {
      // LLGluino samples
      int end_index = sample.find(".root");
      scan_name = sample.substr(0,end_index);
      cout << "LLGluino scan name is " << scan_name << endl;
    }
