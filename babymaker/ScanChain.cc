  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if (evt_id >= 1000 && evt_id < 1200) {
	  ...
	  // LLGluino
	    ...
	    std::cout << "WARNING: expected to find 2 or 3 sparm values, found instead " << sparm_values().size() << std::endl;
	  }
	}
	...
	genTbar_pt = -1.;
	    ...
	    // Suppress this complaint when running on LLGluino sample (3 sparms)
	    if (nHardScatter > 2 && sparm_values().size() != 3) std::cout << "WARNING: found too many gluinos in T1 MC!" << std::endl;
          }
	...
	// For electrons with pT > 50 GeV, save everything
	// regardless of ID and iso
	if (cms3.els_p4().at(iEl).pt() > 50.0) {
	  ...
	  vec_El50noID_eta.push_back ( cms3.els_p4().at(iEl).eta()); //save eta, even though we use SCeta for ID
	  vec_El50noID_phi.push_back ( cms3.els_p4().at(iEl).phi());
	  vec_El50noID_mass.push_back ( cms3.els_mass().at(iEl));
	...
	if (recent_cms3_version) {
      ...
      for(std::vector<std::pair<int, float> >::iterator it = El50noID_pt_ordering.begin(); it!= El50noID_pt_ordering.end(); ++it){
        ...
        if (i >= max_nEl50noID) {
          std::cout << "WARNING: attempted to fill more than " << max_nEl50noID << " leptons" << std::endl;
          break;
        }
